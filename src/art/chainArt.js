...
function jelly(c,x,y,r,value,color,selected,t){
...
export function drawChainScene(c,g){
  c.save();try{
    ...
    oval(c,L.W/2,L.H+L.gu(2),L.W*.8,L.gu(5),'#e9e4bf');
    ...
    const r=g._layout();
    panel(c,L.safe,r.top-L.gu(.1),L.W-L.safe*2,r.step*4+L.gu(.2),'#daeee5',L.gu(.75));
    if(n){
    ...
    g.board.forEach((b,i)=>{
      const p=r.points[i];jelly(c,p.x,p.y,r.r,b.value,COLORS[(i*7+Math.floor(i/4))%4],g.path.includes(i),g.time);
    ...
    const fy=L.zone.floor-L.gu(.12);
    txt(c,g.feedback?.text||(g.wasFever?'무적! 함정 실수도 괜찮아':'3개 ×1.5  ·  5개 ×2  ·  8개 ×3'),L.W/2,fy,L.font(.027),g.feedback?.bad?'#8b4358':INK);
    txt(c,g.keyboardMode?'방향키 이동 · Space 연결 · Enter 팡!':'대각선 OK · 되짚으면 취소',L.W/2,L.H-L.gu(.75),L.font(.023),'#567581');
  }finally{c.restore();}
}
